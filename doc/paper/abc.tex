...
assumptions. 
\chedit{
    Furthermore, it also allows us to infer the posterior using summary
    statistics with likelihoods that are difficult or intractable to directly
    estimate. 
    ABC, therefore, provides a more general inference framework for our forward
...
\begin{center}
    ...
    galaxies with high \sfr.% Based on the posteriors, we find that \emph{galaxies with higher $M_*$ have overall higher dust attenuation and galaxies with higher $\ssfr$ have steeper attenuation curves.  }
    }
\end{center}
\end{figure}
