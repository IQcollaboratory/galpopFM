...
\chedit{
    ...
    randomly sampled $i$, can produce $A_V$ distributions, $p(A_V)$ that 
    match $p(A_V)$ of observations including quiescent galaxies. 
    We, therefore, conclude that the slab model is a sufficiently 
...
\chedit{
    ...
    color as one of our observables.
}
...
\ch{
    ...
    (red).
    All galaxies are edge-on (\ie~$i=0$) and we use an arbitrarily set of
    \eda~parameters ($\mtaum, \mtaus, c_\tau, \mdeltam, \mdeltas, c_\delta$)
    within the prior range listed in Table~\ref{tab:free_param}. 
    In practice the \eda~uniformly samples $\cos i$ from 0 to 1 for each
    galaxy.
}
For comparison, we include the \cite{calzetti2001} attenuation curve. Even for
only edge-on galaxies, the \eda~produces attenuation curves with a wide range
...
\begin{center}
    ...
    \chedit{
        ...
        \eda~$A(\lambda)$ for star-forming galaxies with $\{M_*, \sfr\} =
