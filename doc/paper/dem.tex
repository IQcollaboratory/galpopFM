...
\chedit{
    We use the slab model because it provides a simple prescription for $A_V$
    that depends on a randomly sampled $i$, with loose physical motivations.
    ...
    In this reagrd, we demonstrate that the slab model based \eda~can match the
    observed distribution of $A_V$, even for samples that include quiescent
    galaxies (Appendix~\ref{sec:slab}).
    Hence, the slab model provides an appropriate prescription for sampling
