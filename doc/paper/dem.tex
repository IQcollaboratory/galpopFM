...
For each galaxy, we uniformly sample $\cos i$ from 0 to 1. \tks{By sampling
$\cos i$, we introduce stochasticity in our \eda~prescription, which gives rise to significant variance in
$A(\lambda)$ so galaxies with the same properties do not have identical dust attenuation.}
