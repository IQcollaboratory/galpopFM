...
\chedit{
    In all three simulations with the \eda, quiescent galaxies have
    attenuation curves with overall lower amplitude and shallower slope than
    star forming galaxies.
}

\chedit{
    We predict $A(\lambda)$ with lower attenuation for quiescent galaxies because
    quiescent galaxies in SIMBA, TNG, and EAGLE, without dust, is only slightly
    ...
    Although in SIMBA+\eda, we predict $A(\lambda) \sim 0$, for TNG and EAGLE,
    despite it being lower than star-forming galaxies', there is significant
    dust attenuation in quiescent galaxies, $A_V > 0.5$.
}
...
\chedit{
    We also find that quiescent galaxies have shallower attenuation curves
    than star-formign galaxies. 
    ...
    and~\ref{fig:uv_sfh}).
    These galaxies are quiescent with $\ssfr < 10^{-11}yr^{-1}$ and have high
    ...
    formation than the luminous UV red galaxies we discuss in
    Section~\ref{sec:results}, they have little star formation in the last 1
    Gyr. 
...
\chedit{
    We note that the mass resolution of the simulations impact the SFHs of
    quiescent galaxies and can also impact their observables.
    Since the SFHs of simulated galaxies cannot include any star formation
    below the resolution limit, the SEDs we compute from them do not include
    any star formation below the mass resolution. 
    ...
    in our forward model have $M_* \gtrsim 10^{10}M_\odot$. 
    So even for SIMBA, the effect of mass resolution on $\fnuv$ is
    $<0.1~mag$; for TNG and EAGLE, the effect is even smaller $\lesssim 0.01~mag$. 
    Therefore, mass resolution does not significantly impact the dust
    attenuation we predict for quiescent galaxies.  
}
...
\chedit{
    ...
    in luminous red galaxies not found in SDSS that cannot be
    reconciled by dust attenuation. 
