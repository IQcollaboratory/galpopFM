\begin{figure}
    \begin{center}
        ...
        black dash) compared to $p(A_V)$ of star-forming galaxies \tks{our volume-limited} SDSS \tks{sample}
        (blue) and of $M_* > 10^{10}M_\odot$ galaxies in the \tks{full} \cite{salim2018} GSWLC2 sample (orange). 
        \tks{These samples comprise different, but partially overlapping galaxy populations.} The $A_V$ values for both observations are derived using SED
        fitting\tks{, but using different bands and methodology}~\citep{brinchmann2004, salim2018}. 
        ...
        due to \tks{both differences in the galaxy samples \emph{and}} inconsistencies in the $A_V$ measurements of the two catalogs. 
        \tks{The first highlights the importance of matching observational selections, while the last} illustrates the challenges in observationally measuring $A_V$.
        \tks{Both of these challenges highlight} the advantages of our forward modeling approach.
...
\ch{
    ...
    \tks{While the galaxy populations only partially overlap, this difference is mostly} due to inconsistencies in the $A_V$ measurements of MPA-JHU and
    GSWLC2 --- even for the same galaxy, the $A_V$ measurements from the two
    catalogs differ significantly. \tks{In fact for the ${\sim}8000$ galaxies that are in both samples, the $A_V$ distributions appear similar to those of the corresponding complete samples.}
