...
\chedit{
    ...
    some discrepancies in the color-magnitude relations with SDSS. Furthermore,
    the overall agreement is significantly worse than TNG and EAGLE: the median
    ABC distance metric (Eq.~\ref{eq:distance}) of the posterior $\rho =
    3.7\times10^{-3}, 3.2\times10^{-3}$ for TNG and EAGLE while 
    $\rho = 6.6\times10^{-3}$ for SIMBA. 
    Most notably, SIMBA+\eda~produces a population of galaxies with $\fnuv >
    2.5~mag$ that lie well above the SDSS observations.
}
The inferred \eda~parameters for SIMBA
also differ significantly from the parameters of TNG and EAGLE
(Figure~\ref{fig:abc}). 
\chedit{
    For instance, we infer the opposite $M_*$ and $\ssfr$ dependence for
    $\tau_V$ than TNG or EAGLE.
    ...
    This starburst population, which has also been
    identified in \cite{dave2019} (see their Figures~5 and 6) is caused by
    excess gas in low-mass galaxies at $z=0$. If we exclude $\ssfr >
    10^{-9.5}{yr}^{-1}$ starburst galaxies and run the \eda~for SIMBA using
    median values of the TNG and EAGLE posteriors for the \eda~parameters, we
    find similar level of agreement with SDSS as TNG and EAGLE. 
}
...
\chedit{
    ...
    cannot strongly attenuate and redden just the starburst population.
    Consequently, the quiescent galaxies are not sufficiently attenuated.
    Although we can increase the flexibility of our \eda~prescription, the
    attenuation and reddening necessary for the starburst galaxies conflict
    with the attenuation-slope relation well-established in observations and
    ...
    according to the attenuation-slope relations, galaxies with higher attenuation
    have shallower slopes. 
    Since the excess starburst population is a known discrepancy that
    dramatically impacts the inferred \eda~parameters, we do not explore
    extending our \eda~prescription and focus on the TNG and EAGLE simulations
