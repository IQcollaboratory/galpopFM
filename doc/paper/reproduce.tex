...
\begin{center}
    ...
    $S = A(1500A)/A_V$ as measurements of attenuation and slope, respectively. 
...
\chedit{
    ...
    = A(1500A)/A_V$, commonly found in the literature. The contours mark the 68
    and 95 percentiles. 
    ...
    The \cite{salim2020} GSWLC2 sample extends down to $M_* \sim
...
\begin{center}
    ...
    \chedit{
        ...
        (green).  
        The star-forming galaxies classified using a $\log {\rm SSFR} > -11$ cut. 
        The attenuation curves are normalized at $3000A$ and we mark the
        $1\sigma$ standard deviation of the attenuation curves with the shaded
        region.
        For comparison, we include $A(\lambda)/A(3000A)$ measurements from
...
\chedit{ 
    ...
    In Figure~\ref{fig:sf_atten}, we present the normalized attenuation curves
    of star-forming galaxies predicted by the \eda~for the median posterior
    parameter values of TNG (blue) and EAGLE (green).
    We define galaxies with $\log \ssfr > -11~{yr}^{-1}$ as star-forming.
    The attenuation curves are normalized at $3000A$ and we present the
    variation in the attenuation curves in the shaded region, 1$\sigma$
    standard deviation about the median. 
    For comparison, we include $A(\lambda)/A(3000A)$ from the
    ...
    discrepancies in the amplitudes of UV dust bump. 
