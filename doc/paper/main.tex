...
\begin{abstract}
    ...
    \tksedit{
         ...
         %by building synthetic galaxy surveys and matching the distribution of the galaxy population in observational color-magnitude space.
        ...
        ($M_*$, $\ssfr$), formulated using the \cite{noll2009} attenuation curve. 
    }
    \tksedit{
    We forward model optical and UV
    color-magnitude relations, $(\gr) - M_r$ and $(\fnuv)-M_r$ for three state-of-the-art large-scale 
    cosmological hydrodynamical simulations (SIMBA, IllustrisTNG, and EAGLE) 
    and produce synthetic versions of a $M_r < -20$ complete SDSS galaxy sample. 
    We apply \eda~separately to the three synthetic surveys and compare to SDSS using simulation-based inference.
    As expected, for all three of the simulations dust attenuation is necessary to match the observations.
    }
    ...
    we find good agreement with observed attenuation--slope relations and attenuation curves of star-forming galaxies.
    Additionally, \eda~provides unprecedented predictions for the attenuation curves of quiescent galaxies, which are challenging to measure observationally.
    Interestingly, we find that to match observations, simulated quiescent galaxies require significant UV and optical attenuation but with much shallower
