...
\begin{abstract}
    ...
    The \eda~uses a parameterization of the attenuation curve based on
    observations and a statistical prescription for assigning the curves that 
    includes correlations with galaxy properties. We apply the \eda~to three
    state-of-the-art hydrodynamical simulations --- SIMBA, Illustris
    TNG, and EAGLE --- and forward model their optical and UV color-magnitude
    relations, $(\gr) - M_r$ and $(\fnuv)-M_r$. Then we compare them to SDSS 
    observations using simulation-based inference. For TNG and EAGLE, with the
    \eda~we accurately reproduce the SDSS color magnitude relations. Meanwhile,
    for SIMBA, we struggle to reproduce observations due to an overprediction
    of starburst galaxies. The bestfit \eda~for TNG and EAGLE also predict
    ...
    is challenging to measure in observations. Quiescent galaxies have
    significant UV and optical with shallow attenuation curves. The \eda, more
    broadly, reveals the connections between galaxy properties and dust
    attenuation: more massive galaxies have higher overall dust attenuation
    while galaxies with higher SSFR have steeper attenuation curves. In
    addition to improving our understanding of dust in galaxies, our results
    underscore the advantages of the \eda~and a forward modeling approach. 
\end{abstract}
