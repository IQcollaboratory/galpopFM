...
\begin{abstract}
    ...
    The \eda~uses \tksedit{an observationally motivated parametrization of attenuation}
    ...
    includes correlations with galaxy properties. We apply the \eda~\tksedit{separately} to three
    state-of-the-art hydrodynamical simulations --- SIMBA, \tksedit{IllustrisTNG}, 
    and EAGLE --- and forward model their optical and UV color-magnitude
    relations, $(\gr) - M_r$ and $(\fnuv)-M_r$. Then we compare them to \tksedit{volume-limited SDSS and GALEX 
    ...
    \tksedit{When the \eda~is included we accurately reproduce the color magnitude relations
    of the observational sample for EAGLE and TNG.}
    \tksedit{For} SIMBA, we struggle to reproduce observations due to an overprediction \tks{or overbrightness(?)}
    ...
    is challenging to measure \tksedit{observationally.}
    %in observations. Quiescent 
    \tksedit{We find that quiescent} galaxies have
    significant UV and optical \tksedit{attenuation} with shallow %attenuation 
    curves\tksedit{, in contrast to star-forming galaxies}. The \eda, more
    broadly, reveals the connections between galaxy properties and dust
    attenuation: more massive galaxies %have
    \tksedit{require} higher overall dust attenuation
    while galaxies with higher %SSFR 
    \tksedit{specific star formation rates} have steeper attenuation curves. In
