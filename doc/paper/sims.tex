...
\chedit{
    We also impose a $FUV$ and $NUV$ flux limit --- exclude galaxies
    with galactic-extinction corrected $FUV$ and $NUV$ fluxes below 0.
    UV fluxes in the NASA-Sloan Atlas are measured using forced photometry on
    GALEX so they can be negative. 
    The $\mathtt{kcorrect}$ UV absolute magnitudes derived from these fluxes,
    however, are poorly constrained.
    In addition to the UV flux limit, we also impose a 
    $M_{\rm FUV}, M_{\rm NUV} < -10$
    absolute magnitude completeness limit, which corresponds to the
    $\mathtt{kcorrect}$ absolute magnitude at the flux limit.
    In total, our SDSS sample has 5,341 galaxies.
} 
...
\chedit{
    ...
    Lastly, we apply the $M_r < -20$ and $M_{\rm FUV}, M_{\rm NUV} < -10$
