...
\chedit{
    ...
    Figure~\ref{fig:smf_msfr}}\tksedit{, and are agnostic with respect to galaxies being centrals or satellites. 
}
...
\tksedit{
The SDSS $M_*-\sfr$ relation is strikingly different from those of the simulations. However, these differences are predominantly due to 
the presence of upper limits in the SDSS $\sfr$ measurements and the use of a number of different observational $\sfr$ tracers while the 
simulated values are represented by their theoretical $\sfr$s. Forward modeling the observational $\sfr$ tracers from the simulations provides a 
much more correct comparison with different results (see e.g. \citealp{dickey2020}; Starkenburg et al. in prep.).
Observational aperture effects also affect the $M_*-\sfr$ relation, as well as the SMF. In particular at the higher mass end a significant fraction of the total stellar mass 
of a galaxy can exist at large radii that are not included in observational apertures. For a proper comparison aperture effects should be taken into account \citep{schaye2015, pillepich2018}.
}
...
\chedit{
    We impose a $M_r < -20$ completeness limit on a volume-limited sample from
    \cite{tinker2011}. The \cite{tinker2011} sample is derived from the SDSS
    DR7~\citep{abazajian2009} NYU Value-Added Galaxy
    Catalog~\citep[VAGC;][]{blanton2005} and has a $M_* > 10^{9.4}
    h^{-2}M_\odot$ completeness limit. In total, our SDSS sample has 16,970
    galaxies.
} 
...
\chedit{
    ...
    observables for the simulations. 
    ...
    methods. 
    The comparison can also include selection functions and systematic
    effects of the observations through the forward model. 
...
\chedit{
    ...
    Lastly, we apply the $M_r < -20$ absolute magnitude completeness limit of
    our SDSS sample to the simulated galaxies. 
}
